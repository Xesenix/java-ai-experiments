...
public class TranslateFrom implements IStrategy
{
	private static final ActiveLogger log = new ActiveLogger(LoggerFactory.getLogger(TranslateFrom.class));
	...
	public boolean canPerform(World world, Entity e, IGoal goal)
	{
		...
		return position != null && goal instanceof IPositionGoal;
	}
	...
	public boolean perform(World world, Entity e, IGoal goal)
	{
		...
		if (goal instanceof IPositionGoal)
		{
			...
			return navigation.translateFrom(e, ((IPositionGoal) goal).getTarget(), speed * world.delta);
		}
