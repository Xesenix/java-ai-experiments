...
public class BehaviorTreeNodeDebugSprite extends Group
{
	...
	public void update(IBehavior behavior)
	{
		...
		circle.setFill(behavior.isCompleted() ? (behavior.isSuccess() ? Color.GREEN : Color.RED) : behavior.isRunning() ? Color.YELLOW : behavior.isReady() ? Color.BLUE : Color.BLACK);
		...
		tip.setText(String.format("%s:\n%s\n%s\n%s\n%s", behavior.toString(), behavior.isReady() ? "ready" : "not ready", behavior.isRunning() ? "running" : "not running", behavior.isCompleted() ? "completed" : "not completed", behavior.isSuccess() ? "success" : "fail"));
	}
}
